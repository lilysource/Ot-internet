    ...
    var body: some View {
        NavigationLink { destination() } label: {
            Label {
                ...
                Image(systemName: symbol).foregroundStyle(Color.otBlue)
            }
        }
    }
}
...
struct CalculatorView: View {
    ...
    private func evaluate(_ expression: String) -> String {
        ...
        let offset = expression.unicodeScalars.distance(from: expression.unicodeScalars.startIndex, to: index)
        let operatorIndex = expression.index(expression.startIndex, offsetBy: offset)
